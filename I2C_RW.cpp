...
// SHT I2C Address
int I2C_RW::SHT_I2C_ADDRESS = 0x44;
// SHT I2C Register
int I2C_RW::SHT_REG_TEMP_HUM = 0xE0;
...
int I2C_RW::getShtTempHum(float *temp, float *hum){
    ...
    bool verbose = true;
    String action = "to read temperature and humidity";
    ...
    for(int i = 0; i < maxRetries; i++){
        ...
        Wire.write(SHT_REG_TEMP_HUM);

        Wire.endTransmission(true);

        delay(1);

        Wire.requestFrom(0x44, numBytesToRead);
        

        // Grab the data from the data line
        if (Wire.available() == numBytesToRead) {
            byte req[6];
            Wire.readBytes(req, 6);

            float t_ticks = req[0] * 256 + req[1];
            float rh_ticks = req[3] * 256 + req[4];
